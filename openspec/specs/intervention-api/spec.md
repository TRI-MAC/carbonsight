...
The API SHALL expose `GET /interventions` returning all pre-built intervention types with their category, description, parameters, and default values.

#### Scenario: Client fetches intervention catalog

- **WHEN** client calls `GET /interventions`
- **THEN** response includes all factory interventions (carbon_pricing, ev_subsidy, vmt_reduction, grid_decarbonization, battery_cost_reduction, scrappage_program, phev_charging_improvement) with parameter schemas
