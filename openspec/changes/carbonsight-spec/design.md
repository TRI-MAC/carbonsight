...
**Rationale:** The key insight motivating CarbonSight v2 is that interventions have indirect effects. A carbon tax changes fuel prices, which changes VMT *and* shifts powertrain preferences *and* affects used car market values. Modeling these as causal links in the DAG (rather than independent sliders) captures these propagation effects naturally.
