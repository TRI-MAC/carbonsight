...
def compute_aged_fleet(fleet_inventory: pd.DataFrame) -> pd.DataFrame:
    """DAG compute: age all vehicles by 1 year."""
    return age_fleet(fleet_inventory)
...
def create_fleet_dynamics_nodes(
    ...
    nodes = [
        ...
        Node(
            name="post_new_entry",
            node_type=NodeType.DATAFRAME,
            compute_fn=compute_post_new_entry,
            tags=["fleet", "process"],
        ),
        Node(
        ...
        Node(
            ...
            node_type=NodeType.DATAFRAME,
            compute_fn=compute_post_used_market,
            tags=["fleet", "process"],
        ),
        Node(
            name="fleet_snapshot",
